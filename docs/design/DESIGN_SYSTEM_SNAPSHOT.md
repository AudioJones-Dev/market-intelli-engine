...
| D-9 | Supply the MIOS bibliography — citation markers `[1]`–`[30]` have no reference list. **Constraint RATIFIED 2026-08-01**: MIOS claims are usable as method descriptions but not as report evidence until sourced (`docs/20-MIOS-METHODOLOGY.md` §1). **The bibliography itself remains outstanding** | Operator | Partially closed — constraint binding, input still needed |
| ~~D-10~~ | ~~Ratify `adr/0009-mios-adoption-scope.md`~~ — **RATIFIED 2026-08-01.** ADR-0009 Accepted; MIOS layers 6 and 7 excluded and prohibited | Operator | Closed |
| D-11 | Ratify `adr/0008-mie-domain-boundary.md` — still **Proposed**. ADR-0009 (Accepted) excludes MIOS layers 6–7 *because* ADR-0008 prohibits them, so the pair is currently incoherent | Operator | Recommended — one word closes it |
